...
def compute_factors(df):
    ...
    high = df.get("high", close).astype(float); low = df.get("low", close).astype(float)
    ...
    ag=g.rolling(14).mean(); al=l.rolling(14).mean(); rs=ag/(al+1e-8)
...
def score_stock(df):
    ...
    reasons,risks,rlist=[],[],[]
    if df.empty or len(df)<20: return (0.0,reasons,risks,fs,{"error":"数据不足"},"avoid","high")
    latest=df.iloc[-1]; rows=len(df)
    dq=10
    if rows<60: dq-=5; risks.append(f"仅{rows}条K线")
    if rows<120: dq-=3
    ...
    # risk 20
    rs=20
    if rsi>=80: rs-=8; risks.append(f"RSI过热{rsi:.0f}"); rlist.append("rsi_overheat")
    elif rsi>=70: rs-=5; risks.append(f"RSI偏高{rsi:.0f}"); rlist.append("rsi_high")
    elif rsi<=25: rs-=6; rlist.append("rsi_oversold")
    if ret20>20: rs-=6; rlist.append("chase_risk")
    if vol20>60: rs-=4; risks.append(f"高波动{vol20:.0f}%"); rlist.append("high_volatility")
    if rows<60: rs-=5; rlist.append("insufficient_data")
    fs["risk"]=max(0,rs)
    # pattern 10
    pat=0
    ...
    total=sum(fs.values())
    if total>=75 and fs["risk"]>=14: dec="strong_watch"
    elif total>=55: dec="watch"
    elif total>=30: dec="neutral"
    else: dec="avoid"
    rc=len(rlist)
    if rc>=3 or fs["risk"]<=8: rl="high"
    elif rc>=1 or fs["risk"]<=14: rl="medium"
    else: rl="low"
    return (round(total,1), reasons, risks, fs, fv, dec, rl)
...
class SelectionEngine:
    ...
    def select(self, symbols, start_date="2024-01-01"):
        ...
        for sym in symbols:
            ...
            if df is None or df.empty: results.append({"symbol":sym,"error":"数据获取失败","score":0,"rank":0}); continue
            src=self.fetcher._last_source; rows=len(df)
            df=compute_factors(df)
            score,reasons,risks,fs,fv,dec,rl=score_stock(df)
            lc=_safe_float(df.iloc[-1].get("close"))
            ast=str(df.index[0])[:10] if rows>0 else "N/A"
            aen=str(df.index[-1])[:10] if rows>0 else "N/A"
            results.append({"symbol":sym,"score":score,"reasons":reasons,"risks":risks,"latest_close":round(lc,2),"data_source":src,"rows":rows,"actual_start":ast,"actual_end":aen,"factor_scores":fs,"factor_values":fv,"decision":dec,"risk_level":rl})
