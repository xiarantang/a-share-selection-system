...
如需在网络不稳定时多一层保障，可双击 **`scripts/install_fallback.command`** 安装第三级兜底数据通道（可选，约 1 分钟）。
...
> 维护者/发布前验收用，聚合语法检查、UI 验收、策略验收、CLI 选股/报告、run_metadata 复盘记录、排障体验、文档口径一致性等 12 项检查。不是小白日常使用步骤。
