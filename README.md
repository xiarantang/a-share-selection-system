...
基于 **akshare + skill fallback + backtrader** 的 A 股选股系统。Streamlit 本地可视化界面，三步完成选股，含验证、复盘、报告和排障指南。**仅供研究学习，不构成投资建议。**
...
> 上图：首页四步引导 + 环境自检 + 就绪状态（左）；选股完成后的首屏会直接显示完成状态、数据覆盖和覆盖不足说明，候选表格与验证摘要可在页面继续查看。
