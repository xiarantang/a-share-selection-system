...
> **文档分层说明**：第 1–7 节为**打标前设计记录**（记录 P9.5 规划和执行过程），第 7.1 节为**打标后状态确认**（记录最终事实）。当前真实状态以第 7.1 节为准。
...
推荐 `v0.6-rc1`。但最终是否打 tag、何时打、用什么版本号，仍需人工确认。本文档不抢先 tag。（打标前设计记录：该建议已由人工确认并执行，见 7.1 节。）
...
推荐 tag 版本：`v0.6-rc1`。建议 tag 命令已记录在 PROJECT_STATE.md，由人工确认后手动执行。本文档未抢先打 tag，不把 tag 当成功证明。（打标前记录：该建议已由人工确认并执行，见 7.1 节。）

---

## 7.1 打标后状态确认

v0.6-rc1 tag 已于 2026-05-19 由人工确认后创建并推送到远端：

| 项目 | 值 |
|------|-----|
| tag | `v0.6-rc1` |
| 指向 commit | `3461390`（P9.5-3.1: 最终验收记录 commit 号同步） |
| 远端状态 | `refs/tags/v0.6-rc1` 已推送至 origin |
| 发布前验收 | 12/12 / 19/19 / 20/20 / 13/13 全部通过 |

P9.5 发布准备阶段已完成，tag 已打标。后续如需修正可出 `v0.6-rc2` 或直接 `v0.6`。
