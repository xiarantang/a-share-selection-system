...
| D-2 | app.py 选股全部失败时，error 直接暴露 `技术详情: {e}`（Python traceback） | 小白看到技术报错容易恐慌 |
...
6. **禁词红线**：不使用"买入/卖出/目标价/收益预测"等投资建议措辞。
...
| 阶段 | 内容 | 改动范围 | 验收方式 |
|------|------|----------|----------|
| P9.4-1 | 公开排障文档口径收口 | docs/TROUBLESHOOTING.md / docs/USER_GUIDE.md | confirm_docs_consistency + confirm_release_ready |
| P9.4-2 | UI 错误/等待/空结果提示增强 | app.py（只改 inline 提示文案和布局） | py_compile + confirm_release_ready 11/11 |
