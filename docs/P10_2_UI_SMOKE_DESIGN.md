...
主要改动：
1. 首页关键词：旧步骤计数词 → 移除（当前 UI 不使用步骤计数）
2. 结果页关键词：原始英文决策标签 → 替换为中文决策标签检查（至少出现一种：强观察/观察/中性/回避）
...
**实际复核结果**（P10.2-2 + P10.2-2.1 修正）：

本地 Streamlit 启动，screenshot_home.py 从真实页面截图并检查关键词。

截图已更新：
- `docs/screenshots/home.png`：~183KB → ~198KB（从真实 Streamlit 首页截取）
- `docs/screenshots/result.png`：~147KB → ~185KB（从真实 Streamlit 结果页截取）

关键词检查结果（P10.2-2.1 修正后重跑，退出码 0）：
- 首页 5/5 通过：A股智能选股 ✅、开始选股 ✅、系统就绪 ✅、环境检查 ✅、免责声明 ✅
- 结果页关键词全部通过：选股完成、候选表格、数据区间、验证摘要、整体质量、免责声明
- 决策标签中文展示确认（强观察、观察、中性）
- 脚本退出码 0

P10.2-2.1 修正说明：P10.2-2 首页 2 项旧关键词（「可以开始选股」「首次使用检查」）与真实 UI 文案有微小差异，已替换为真实 UI 稳定文本（「系统就绪」「环境检查」），重跑通过。
...
- PROJECT_STATE.md
- docs/P10_ROADMAP.md
- docs/P10_2_UI_SMOKE_DESIGN.md

禁止修改：
- 产品代码 / scripts/ / docs/screenshots/

**P10.2 完成结论**：

P10.2 全部子阶段已完成：P10.2-0 设计/诊断 → P10.2-1 截图脚本关键词修正 → P10.2-1.1 白名单扩展 → P10.2-2 真实截图复核 → P10.2-2.1 首页关键词修正并重跑 → P10.2-3 文档收口。

最终结论：
- 真实 Streamlit 可启动，screenshot_home.py 关键词已对齐真实 UI 并退出码 0
- 截图已更新：home.png ~198KB、result.png ~185KB（从真实 Streamlit 页面截取）
- 未修改产品代码、评分、排序、数据链路
- 下一步：P10.3 发布后问题记录模板
