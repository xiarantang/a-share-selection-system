...
| P10.4-2 | — | 修正正式版决策记录 commit 口径 |
| P10.4-1 | — | 同步正式版决策记录 main 最新 commit 口径 |
...
- 新增 docs/P10_ROADMAP.md：P10 路线图
  ...
  - 当前基线：P10.4 决策基线 commit `f670716`、v0.6-rc1 tag 指向 `3461390`、12/12 通过（当前 main 最新提交以 git log 为准）
...
P10.2-2 真实 UI 截图复核并更新截图：
- 本地 Streamlit 启动，运行 screenshot_home.py 执行真实截图复核
- 截图已更新：
  - docs/screenshots/home.png：~183KB → ~198KB（从真实 Streamlit 首页截取）
  - docs/screenshots/result.png：~147KB → ~185KB（从真实 Streamlit 结果页截取）
- Streamlit 后台进程已停止，端口已释放
- 未修改 app.py / main.py / start_ui.command / strategies/ / data/ / reports/ / validation/ / 评分/排序/数据链路
- 不直写投资建议类禁词

P10.2-2.1 修正截图脚本首页关键词并重跑通过：
- 问题：P10.2-2 首页 2 项关键词（「可以开始选股」「首次使用检查」）与真实 UI 文案有微小差异，脚本退出码 1
- 修正：screenshot_home.py 首页关键词替换为真实 UI 稳定文本
  - 「可以开始选股」→「系统就绪」
  - 「首次使用检查」→「环境检查」
- 重跑结果（端口 8501）：
  - 首页 5/5 通过：A股智能选股 ✅、开始选股 ✅、系统就绪 ✅、环境检查 ✅、免责声明 ✅
  - 结果页关键词全部通过：选股完成、候选表格、数据区间、验证摘要、整体质量、免责声明
  - 决策标签中文：强观察、观察、中性
  - 脚本退出码 0
- 截图重跑后 home.png / result.png 有更新
- Streamlit 后台进程已停止，端口 8501 已释放
- 未修改 app.py / main.py / start_ui.command / strategies/ / data/ / reports/ / validation/ / 评分/排序/数据链路
- 不直写投资建议类禁词
- 下一步建议：P10.2-3 文档收口

P10.2-3 真实 UI 截图复核文档收口：
- P10.2 完成结论：真实 Streamlit 可启动，screenshot_home.py 关键词已对齐真实 UI 并退出码 0，home.png/result.png 已更新，后台服务已关闭
- P10.2 全部子阶段完成：P10.2-0 设计/诊断 → P10.2-1 截图脚本关键词修正 → P10.2-1.1 白名单扩展 → P10.2-2 真实截图复核 → P10.2-2.1 首页关键词修正并重跑 → P10.2-3 文档收口
- 未修改 app.py / main.py / start_ui.command / strategies/ / data/ / reports/ / validation/ / 评分/排序/数据链路
- 不直写投资建议类禁词
- 下一步建议：P10.3 发布后问题记录模板

P10.3 新增发布后问题记录模板：
- 新增 docs/POST_RELEASE_NOTES.md：发布后观察期问题反馈记录模板
  - 文档目的：记录发布后问题、反馈、复现步骤、处理状态
  - 使用原则：只记录问题和复盘，不改变评分/排序/数据链路
  - 问题分级：阻断 / 重要 / 一般 / 观察
  - 状态定义：待复现 / 已确认 / 处理中 / 已修复 / 暂不处理 / 已关闭
  - 记录表格字段：编号、日期、来源、页面/功能、问题描述、复现步骤、期望表现、实际表现、影响范围、分级、状态、关联阶段、处理记录
  - 含空白模板行 P-001，方便照着填写
  - 保留免责声明：仅供研究学习，不构成投资建议
- 未修改 app.py / main.py / start_ui.command / scripts/ / strategies/ / data/ / reports/ / validation/ / 评分/排序/数据链路
- 不直写投资建议类禁词
...
- 新增 docs/P10_4_RELEASE_DECISION.md：v0.6 正式版决策记录
  - 当前基线：v0.6-rc1 tag 指向 commit 3461390，P10.4 决策基线 commit f670716（当前 main 最新提交以 git log 为准）
...
P10.4-1 同步正式版决策记录 main 最新 commit 口径：
- 将决策文档、路线图、PROJECT_STATE.md 中的旧 commit hash 同步为 P10.4 提交 f670716
- 未修改产品代码、评分、排序、数据链路
- 不直写投资建议类禁词

P10.4-2 修正正式版决策记录 commit 口径：
- 问题：每次提交后"main 最新 commit"写死值就会过时，形成自引用死循环
- 修正：将"main 最新 commit"改为"P10.4 决策基线 commit"，值锚定在 f670716（P10.4 决策记录提交）
- 增加说明：当前 main 最新提交以后续 git log 为准，P10.4-1/P10.4-2 为文档口径收口
- 未修改产品代码、评分、排序、数据链路
- 不直写投资建议类禁词
